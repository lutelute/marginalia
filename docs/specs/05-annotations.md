...
- **返信スレッド**: 注釈への返信チェーン
- **孤立注釈検出**: ドキュメント変更時に未アンカー注釈を自動検出（AnnotationProvider が常時監視、パネル表示に非依存）
- **孤立注釈の再アサイン**: 新しいテキスト位置に手動再割当て
- **外部変更への追従**: 外部編集（git pull・他エディタ等）検出 → 再読み込みで `.md` と `.mrgl` の両方をディスクから再読込し、注釈を再アンカー
- **`.mrgl` 単独変更の自動反映**: 注釈ファイルだけが外部で変わった場合（注釈のみの git pull 等）はサイレント再読込＋トースト通知（.marginalia ディレクトリ監視、後付け作成も検出）
- **マルチユーザー author**: 注釈・返信の author に設定（ユーザータブ）の現在ユーザー名と authorId を記録（v2.1.0）。表示は色バッジ付き、未知の作成者はグレー
- **構造化履歴**: 追加・更新・削除・返信・解決/解除・保持・再割当て・孤立検出を、操作者（author/authorId）と前後テキスト（detail）付きで記録。サイズ上限512KB
- **差分×注釈オーバーレイ**: バックアップ差分ビューの該当行に注釈マーカーをガター表示
...
- **authorId はローカル設定由来**: authorId は各環境の設定が生成した ID で、中央管理されない。別環境の同一人物を ID で同定するには各自の users 設定を揃える運用が必要
- **ブロック注釈**: `blockId` による注釈は実装途中。DOM ブロックへのアンカリングロジックが不完全
- **本文の外部変更の反映は手動承認**: `.md` の外部変更は fs.watch + 5秒ポーリングで検出し通知バナーを表示するが、再読込はユーザーが「再読み込み」を押したときに行う（未保存編集の暗黙破棄を避けるため。`.mrgl` 単独変更は本文に影響しないため自動反映）
- **履歴のサイズ上限**: シリアライズ後512KBで古い順に切り捨て（truncateHistoryBySize）。それ以前の履歴を保全する仕組みはない
...
### Phase 2: 本格的な実装
- ~~マルチユーザー対応（author フィールドの設定可能化）~~ → 実装済（設定の現在ユーザー名を記録）。残: ユーザーIDによる同一性管理
