...
- **マルチユーザー author**: 注釈・返信の author に設定（ユーザータブ）の現在ユーザー名を記録
...
- **ユーザー識別は表示名ベース**: author には設定の現在ユーザー名（文字列）を記録する。ID ではないため、同名ユーザーの区別や名前変更後の同一性追跡はできない
- **ブロック注釈**: `blockId` による注釈は実装途中。DOM ブロックへのアンカリングロジックが不完全
- **外部変更の反映は手動承認**: 外部変更は fs.watch + 5秒ポーリングで検出し通知バナーを表示するが、`.md`/`.mrgl` の再読込はユーザーが「再読み込み」を押したときに行う（未保存編集の暗黙破棄を避けるため自動リロードはしない）
- **`.mrgl` 単独の外部変更は未検出**: 監視対象は開いている `.md` のみ。`.mrgl` だけが外部で変わった場合（md 変更を伴わない注釈のみの更新）は通知されない
